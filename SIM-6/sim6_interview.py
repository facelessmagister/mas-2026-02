#!/usr/bin/env python3
"""
SIM-6 Agent Interview Tool

Load a persona from sim6_personas.json and conduct an interactive interview
with the agent after simulation completion. The agent retains all memories
and final opinion state from the simulation.

Usage:
    python3 sim6_interview.py --agent "Dr Lim Kah Wai"
    python3 sim6_interview.py --agent agent_0006
    python3 sim6_interview.py --list
...
API_KEY = "ollama"
PERSONAS_PATH = Path("/root/projects/mas-2026-02/SIM-6/sim6_personas.json")
...
You have just survived a policy simulation (SIM-6: Private Sector Integration) from 2026-2030.
Malaysia adopted a market-based approach: private sector expanded training, compulsory 3-year public service then free market, budget offloaded to private sector, government vouchers for poor patients.
...
# ---------------------------------------------------------------------------
def main() -> None:
    parser = argparse.ArgumentParser(description="Interview a SIM-6 agent")
    ...
    if not args.agent:
        print("Usage: python3 sim6_interview.py --agent 'Dr Lim Kah Wai'")
        print("       python3 sim6_interview.py --list")
        sys.exit(1)
