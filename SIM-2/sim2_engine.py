#!/usr/bin/env python3
"""
SIM-2: Malaysia Doctor Shortage — Intake Surge (Repeat 1990s Mistake)
...
Policy premise: Government panics and rapidly expands medical school intake
to 8,000+/year, opens new schools. No parallel post creation. This repeats
the exact cycle that produced the 2016 glut.

Outputs:
- sim2_log.jsonl          — per-round agent actions
- sim2_state.json         — quantitative state trajectory
- sim2_personas.json      — agent personas for post-sim interviews
- sim2_report.md          — structured markdown report
"""
...
OUT_DIR = Path("/root/projects/mas-2026-02/SIM-2")
...
- Housemanship waiting list (months): {ho_waitlist}
- Unemployed medical graduates: {unemployed_grads}
...
STATE_0 = {
    ...
    "retention": 58,
    "ho_waitlist": 3,
    "unemployed_grads": 1200,
}

# ---------------------------------------------------------------------------
# State update formulas (SIM-2: Intake Surge — repeats 1990s mistake)
# ---------------------------------------------------------------------------
def update_state(state: Dict[str, float], round_num: int) -> Dict[str, float]:
    s = dict(state)
    s["year"] = 2026.0 + round_num * 0.5

    # INTAKE SURGE: +10%/year toward 8,000 cap by round 4 (2028)
    target_intake = 8000
    if s["graduates"] < target_intake:
        s["graduates"] = min(target_intake, int(s["graduates"] * 1.10))

    # Posts grow insufficiently
    s["housemanship_posts"] += s["housemanship_posts"] * 0.03  # +3%/year (insufficient)
    s["permanent_posts"] += s["permanent_posts"] * 0.01  # +1%/year (negligible)

    # Massive contract expansion to absorb overflow
    overflow = s["graduates"] - s["housemanship_posts"]
    s["contract_doctors"] = max(0, s["contract_doctors"] + overflow * 0.8)

    # Housemanship waitlist grows as graduates outstrip posts
    s["ho_waitlist"] = max(0, (overflow * round_num * 0.3) / 12)

    # Unemployed graduates pile up
    s["unemployed_grads"] = max(0, overflow * round_num * 0.5)

    # Brain drain worsens as frustration grows from glut conditions
    s["brain_drain"] = 1500 + round_num * 50
    s["returnees"] = max(20, 50 - round_num * 5)  # deterred by glut + poor conditions

    # Interest initially spikes (shortage panic) then crashes (glut reality)
    if round_num <= 2:
        s["interest"] = min(3.0, s["interest"] + round_num * 0.15)  # brief hope
    else:
        s["interest"] = max(0.8, s["interest"] - (round_num - 2) * 0.20)  # then crash

    # Doc ratio: initially improves slightly from sheer numbers, then stagnates as brain drain offsets
    if round_num <= 3:
        s["doc_ratio"] = min(2.4, 2.3 + round_num * 0.03)
    else:
        s["doc_ratio"] = max(1.8, 2.4 - (round_num - 3) * 0.05)

    # Overcrowding: initially improves as more HO enter, then worsens as contract system chaos
    if round_num <= 2:
        s["overcrowding"] = max(60, 78 - round_num * 4)
    else:
        s["overcrowding"] = min(100, 70 + (round_num - 2) * 6)

    # Workload: dips briefly then surges as system absorbs too many trainees
    if round_num <= 2:
        s["workload"] = max(65, 72 - round_num * 2)
    else:
        s["workload"] = min(95, 68 + (round_num - 2) * 4)

    s["kk_wait"] = min(30, 14 + round_num * 1.2)
    s["specialist_wait"] = min(24, 12 + round_num * 1.2)

    # Budget: slight cut then partial reversal as crisis spending, then reimposed
    if round_num <= 3:
        s["health_budget"] = max(1.8, 2.1 - round_num * 0.02)
    else:
        s["health_budget"] = max(1.6, 1.95 - (round_num - 3) * 0.03)

    s["salary_ratio"] = max(0.30, 0.45 - round_num * 0.015)
    s["retention"] = max(20, 58 - round_num * 5)
    return s
...
# ---------------------------------------------------------------------------
EVENTS: Dict[int, str] = {
    1: "Budget 2027: 5% health cut (less severe than SIM-1 due to crisis optics). Government announces 'Medical Education Expansion Initiative' — 5 new medical schools approved.",
    2: "MOHE announces intake will rise to 6,500 by 2027 and 8,000 by 2028. JPA warns no post budget allocated. Health DG calls it 'irresponsible without workforce planning'.",
    3: "250 MOs resign from HKL. Simultaneously, 2027 graduating class hits 5,500 — housemanship waitlist grows to 6 months. First signs of incoming glut.",
    4: "Election 2028: Opposition warns of '2016 glut 2.0'. Government claims 'we need more doctors, not less'. Intake reaches 7,200. JPA silent.",
    5: "Post-election: government retains reduced majority. Expands contract system to 'absorb' surplus graduates. Contract doctors now 25,000+. Perm posts unchanged.",
    6: "World Bank report: Malaysia has high doctor density but severe maldistribution. Urban hospitals flooded with HO, rural areas still empty. Training quality concerns raised.",
    7: "Singapore announces accelerated visa. Malaysian doctors now include fresh graduates who completed housemanship but cannot get permanent posts. 500 apply in first month.",
}
...
def build_prompt(agent: Agent, state: Dict[str, float], round_num: int, timeline: str, observations: str = "") -> str:
    ...
    return SIM_PROMPT_TEMPLATE.format(
        ...
        ho_waitlist=round(state["ho_waitlist"], 1),
        unemployed_grads=int(state["unemployed_grads"]),
...
async def run_simulation(agents: List[Agent]) -> tuple[List[Dict], List[Dict]]:
    ...
    for r in range(1, ROUNDS + 1):
        ...
        timeline = f"System state: intake surging to {state['graduates']}, waitlist {state['ho_waitlist']:.1f} months."
        if r > 1:
...
def generate_report(agents: List[Agent], log_entries: List[Dict], state_trajectory: List[Dict]) -> str:
    ...
    for s in state_trajectory:
        lines.append(
            ...
            f"{s['ho_waitlist']:.1f} | {s['unemployed_grads']:.0f} |"
        )

    lines.extend(["", "### Key Trends", ""])
    s0 = state_trajectory[0]
    s_last = state_trajectory[-1]
    lines.append(f"- **Graduate intake:** {s0['graduates']} -> {s_last['graduates']} (massive expansion)")
    lines.append(f"- **Housemanship waitlist:** {s0['ho_waitlist']:.1f} -> {s_last['ho_waitlist']:.1f} months (bottleneck)")
    lines.append(f"- **Unemployed graduates:** {s0['unemployed_grads']} -> {s_last['unemployed_grads']} (glut forming)")
    lines.append(f"- **Doctor-to-population ratio:** {s0['doc_ratio']:.2f} -> {s_last['doc_ratio']:.2f} (initial rise then crash)")
    lines.append(f"- **Public hospital overcrowding:** {s0['overcrowding']:.0f} -> {s_last['overcrowding']:.0f} (reversal then surge)")
    lines.append(f"- **Average doctor workload:** {s0['workload']:.0f} -> {s_last['workload']:.0f} hours/week (chaos)")
    lines.append(f"- **Brain drain:** {s0['brain_drain']:.0f} -> {s_last['brain_drain']:.0f} doctors/year (accelerating)")
    lines.append(f"- **Medical student interest:** {s0['interest']:.2f} -> {s_last['interest']:.2f} (boom then bust)")
    lines.append(f"- **Retention:** {s0['retention']:.0f}% -> {s_last['retention']:.0f}% (mass exodus)")
    lines.append("")
    ...
    lines.extend([
        ...
        "",
        "---",
        f"*Report generated by SIM-2 engine | {datetime.now().isoformat()}*",
    ])
...
async def main():
    random.seed(SEED)
    print(f"SIM-2 Starting... Agents: {AGENTS_COUNT}, Rounds: {ROUNDS}, Model: {MODEL_NAME}")
    ...
    personas_path = OUT_DIR / "sim2_personas.json"
    ...
    log_path = OUT_DIR / "sim2_log.jsonl"
    with log_path.open("w", encoding="utf-8") as f:
        for entry in log_entries:
            f.write(json.dumps(entry, ensure_ascii=False) + "\n")
    print(f"Log saved to {log_path}")

    state_path = OUT_DIR / "sim2_state.json"
    ...
    report_path = OUT_DIR / "sim2_report.md"
    report_path.write_text(report, encoding="utf-8")
    print(f"Report saved to {report_path}")

    print("\nSIM-2 complete. To interview an agent, load sim2_personas.json and run the interview script.")
