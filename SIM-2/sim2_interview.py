#!/usr/bin/env python3
"""
SIM-2 Agent Interview Tool

Load a persona from sim2_personas.json and conduct an interactive interview
with the agent after simulation completion. The agent retains all memories
and final opinion state from the simulation.

Usage:
    python3 sim2_interview.py --agent "Dr Lim Kah Wai"
    python3 sim2_interview.py --agent agent_0006
    python3 sim2_interview.py --list
...
API_KEY = "ollama"
PERSONAS_PATH = Path("/root/projects/mas-2026-02/SIM-2/sim2_personas.json")
...
def load_personas() -> List[Dict[str, Any]]:
    if not PERSONAS_PATH.exists():
        ...
        print("Run sim2_engine.py first to generate personas.")
        sys.exit(1)
...
def interview_agent(persona: Dict[str, Any]) -> None:
    ...
    system_msg = (
        ...
        "You have just completed a simulation of Malaysia's doctor shortage crisis under the INTAKE SURGE policy. "
        "The government rapidly expanded medical school intake to 8,000+/year without creating posts, repeating the 1990s cycle. "
        ...
        "Respond authentically as your character. Use Malaysian English / Malay phrases where natural. "
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Interview SIM-2 agents after simulation.")
    ...
    if not args.agent:
        print("Usage: python3 sim2_interview.py --agent 'Dr Lim Kah Wai'")
        print("       python3 sim2_interview.py --list")
        sys.exit(1)
