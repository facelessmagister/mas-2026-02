#!/usr/bin/env python3
"""
SIM-3 Agent Interview Tool

Load a persona from sim3_personas.json and conduct an interactive interview
with the agent after simulation completion. The agent retains all memories
and final opinion state from the simulation.

Usage:
    python3 sim3_interview.py --agent "Dr Lim Kah Wai"
    python3 sim3_interview.py --agent agent_0006
    python3 sim3_interview.py --list
...
API_KEY = "ollama"
PERSONAS_PATH = Path("/root/projects/mas-2026-02/SIM-3/sim3_personas.json")
...
def load_personas() -> List[Dict[str, Any]]:
    if not PERSONAS_PATH.exists():
        ...
        print("Run sim3_engine.py first to generate personas.")
        sys.exit(1)
...
def interview_agent(persona: Dict[str, Any]) -> None:
    ...
    system_msg = (
        ...
        "You have just completed a simulation of Malaysia's doctor shortage crisis (2026-2030) under the BALANCED PIPELINE policy. "
        "In this scenario, intake rose moderately to 6,500 but only in lockstep with post creation. Contract doctors were converted to permanent posts. Salaries rose 15%. The budget was protected.\n\n"
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Interview SIM-3 agents after simulation.")
    ...
    if not args.agent:
        print("Usage: python3 sim3_interview.py --agent 'Dr Lim Kah Wai'")
        print("       python3 sim3_interview.py --list")
        sys.exit(1)
