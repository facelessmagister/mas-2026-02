#!/usr/bin/env python3
"""
SIM-3: Malaysia Doctor Shortage — Balanced Pipeline (Match Intake to Posts)
...
Policy premise: Intake increases moderately (4,820 -> 6,500) but ONLY in lockstep
with verified post creation. Housemanship posts grow 1:1 with graduate projections.
Permanent posts expand (+8%/year). Contract system phased out over 10 years.
Salary rises +15% by 2028. Budget protected. Brain drain falls to 800/year.

Outputs:
- sim3_log.jsonl          — per-round agent actions
- sim3_state.json         — quantitative state trajectory
- sim3_personas.json      — agent personas for post-sim interviews
- sim3_report.md          — structured markdown report
"""
...
OUT_DIR = Path("/root/projects/mas-2026-02/SIM-3")
...
# ---------------------------------------------------------------------------
# State update formulas (SIM-3: Balanced Pipeline)
# ---------------------------------------------------------------------------
def update_state(state: Dict[str, float], round_num: int) -> Dict[str, float]:
    s = dict(state)
    s["year"] = 2026.0 + round_num * 0.5

    # Intake grows moderately toward 6,500 by round 5 (2028), then holds
    target_intake = 6500
    if s["graduates"] < target_intake:
        s["graduates"] = min(target_intake, int(s["graduates"] * 1.065))

    # Housemanship posts grow in lockstep with graduate projections (+7.5%/round ~ +15%/year)
    s["housemanship_posts"] = min(s["graduates"], int(s["housemanship_posts"] * 1.075))

    # Permanent posts expand +4%/round (+8%/year)
    prev_permanent = s["permanent_posts"]
    s["permanent_posts"] = int(s["permanent_posts"] * 1.04)
    new_permanent = s["permanent_posts"] - prev_permanent

    # Contract system phased out: converts faster than new posts alone
    conversion = int(new_permanent * 2.5)
    s["contract_doctors"] = max(5000, s["contract_doctors"] - conversion)

    # Brain drain improves as certainty returns
    s["brain_drain"] = max(800, 1200 - round_num * 50)

    # Returnees improve as conditions improve
    s["returnees"] = min(200, 80 + round_num * 15)

    # Doc ratio improves steadily
    s["doc_ratio"] = min(2.6, 2.3 + round_num * 0.03)

    # Overcrowding eases gradually
    s["overcrowding"] = max(50, 78 - round_num * 2.5)

    # Workload eases
    s["workload"] = max(55, 72 - round_num * 1.8)

    # Waiting times improve
    s["kk_wait"] = max(7, 14 - round_num * 0.9)
    s["specialist_wait"] = max(6, 12 - round_num * 0.8)

    # Interest: initial dip (reform uncertainty), then recovery
    if round_num <= 2:
        s["interest"] = max(1.5, 1.8 - round_num * 0.05)
    else:
        s["interest"] = min(2.2, 1.7 + (round_num - 2) * 0.06)

    # Budget protected (flat in real terms)
    s["health_budget"] = 2.1

    # Salary ratio improves (+15% by 2028 ~ +3.75%/round)
    s["salary_ratio"] = min(0.55, 0.45 + round_num * 0.018)

    # Retention improves
    s["retention"] = min(72, 58 + round_num * 1.8)
...
# ---------------------------------------------------------------------------
EVENTS: Dict[int, str] = {
    1: "Budget 2027: Health budget protected from cuts after MMA campaign and DG advocacy. Treasury agrees to freeze health allocation in real terms.",
    2: "MOHE announces measured intake increase to 5,800 by 2027, with JPA committing to parallel housemanship post creation. First MOHE-JPA coordination agreement signed.",
    3: "Government launches 'Graduate-to-Post Matching Initiative'. 1,500 contract doctors converted to permanent posts in first wave. MMA cautiously supportive.",
    4: "Election 2028: Health workforce gains bipartisan support. Opposition echoes call for Health Services Commission. No reversal threats.",
    5: "Post-election: New government continues balanced pipeline policy. Contract conversion accelerates to 3,000/year. Salary adjustment bill (+15%) passes cabinet.",
    6: "World Bank report: Malaysia shows 'promising trajectory' in health workforce planning. Cites intake-post coordination as regional best practice.",
    7: "Singapore reports slower Malaysian doctor emigration. 200 doctors return from UK/Australia citing improved career pathways and permanent post guarantees.",
}
...
async def run_simulation(agents: List[Agent]) -> tuple[List[Dict], List[Dict]]:
    ...
    for r in range(1, ROUNDS + 1):
        ...
        timeline = f"System state improving under balanced pipeline: intake {state['graduates']}, posts {state['housemanship_posts']:.0f}, contracts falling."
        if r > 1:
...
def generate_report(agents: List[Agent], log_entries: List[Dict], state_trajectory: List[Dict]) -> str:
    ...
    lines.append(f"- **Graduate intake:** {s0['graduates']} -> {s_last['graduates']} (measured expansion)")
    lines.append(f"- **Housemanship posts:** {s0['housemanship_posts']:.0f} -> {s_last['housemanship_posts']:.0f} (matched to graduates)")
    lines.append(f"- **Permanent posts:** {s0['permanent_posts']:.0f} -> {s_last['permanent_posts']:.0f} (major expansion)")
    lines.append(f"- **Contract doctors:** {s0['contract_doctors']:.0f} -> {s_last['contract_doctors']:.0f} (phased out)")
    lines.append(f"- **Doctor-to-population ratio:** {s0['doc_ratio']:.2f} -> {s_last['doc_ratio']:.2f} (steady improvement)")
    lines.append(f"- **Public hospital overcrowding:** {s0['overcrowding']:.0f} -> {s_last['overcrowding']:.0f} (easing)")
    lines.append(f"- **Average doctor workload:** {s0['workload']:.0f} -> {s_last['workload']:.0f} hours/week (improving)")
    lines.append(f"- **Brain drain:** {s0['brain_drain']:.0f} -> {s_last['brain_drain']:.0f} doctors/year (declining)")
    lines.append(f"- **Overseas returnees:** {s0['returnees']:.0f} -> {s_last['returnees']:.0f} doctors/year (rising)")
    lines.append(f"- **Medical student interest:** {s0['interest']:.2f} -> {s_last['interest']:.2f} (recovering)")
    lines.append(f"- **Retention:** {s0['retention']:.0f}% -> {s_last['retention']:.0f}% (improving)")
    lines.append("")
    ...
    lines.extend([
        ...
        "",
        "---",
        f"*Report generated by SIM-3 engine | {datetime.now().isoformat()}*",
    ])
...
async def main():
    random.seed(SEED)
    print(f"SIM-3 Starting... Agents: {AGENTS_COUNT}, Rounds: {ROUNDS}, Model: {MODEL_NAME}")
    ...
    personas_path = OUT_DIR / "sim3_personas.json"
    ...
    log_path = OUT_DIR / "sim3_log.jsonl"
    with log_path.open("w", encoding="utf-8") as f:
        for entry in log_entries:
            f.write(json.dumps(entry, ensure_ascii=False) + "\n")
    print(f"Log saved to {log_path}")

    state_path = OUT_DIR / "sim3_state.json"
    ...
    report_path = OUT_DIR / "sim3_report.md"
    report_path.write_text(report, encoding="utf-8")
    print(f"Report saved to {report_path}")

    print("\nSIM-3 complete. To interview an agent, load sim3_personas.json and run the interview script.")
